public class Player {
    private static double rotationValue = 0.1;
    private static int movementValue = 1;
    ...
    public static double[][] endPointsOfVectors = new double[Raycasting.numberOfRays][2];
    ...
    private static void move(int scalar) {
        double newX = x + dirX*scalar;
        double newY = y + dirY*scalar;

        if(newX > 0 && newX < Constants.windowWidth && !Constants.map[(int) (newX/Constants.cellWidth)][(int) (newY/Constants.cellHeight)]) x = newX;
        if(newY > 0 && newY < Constants.windowHeight && !Constants.map[(int) (newX/Constants.cellWidth)][(int) (newY/Constants.cellHeight)]) y = newY;
    }
